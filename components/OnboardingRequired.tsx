...
export function OnboardingRequired({ destination = "assignment" }: { destination?: "assignment" | "plan" }) {
  return (
    <section className="border-y border-[var(--line)] bg-[var(--surface-soft)] px-5 py-7" aria-labelledby="setup-required-heading">
      <h2 id="setup-required-heading" className="text-xl font-semibold tracking-[-0.03em]">Start with your recurring week.</h2>
      <p className="mt-2 max-w-xl text-sm leading-6 text-[var(--muted-ink)]">PlanAround needs your normal classes and weekly commitments before it can build {destination === "plan" ? "a realistic assignment plan" : "an assignment around your actual availability"}.</p>
